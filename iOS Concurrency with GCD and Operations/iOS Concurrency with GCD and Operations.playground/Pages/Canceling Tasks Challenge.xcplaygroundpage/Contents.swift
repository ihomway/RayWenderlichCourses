			...
			operation.completionBlock = {
				...
				if let output = operation.output {
					self.appendQueue.addOperation({ 
						...
						self.outputArray.append((operation.inputPair.0, operation.inputPair.1, output))
					})
				}
...
startClock()
groupAdd.start()
//: __TEST:__ Uncomment this call to `cancel()` to check that you've implemented it correctly. You should see the `stopClock()` result reduce, as should the length of the output.
groupAdd.cancel()
