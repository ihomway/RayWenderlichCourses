...
func asyncTiltShift(image: UIImage?, workQueue:DispatchQueue, resultQueue: DispatchQueue, completion:@escaping (UIImage?) -> ()) {
...
duration {
	asyncTiltShift(image: image, workQueue: workerQueue, resultQueue:resultQueue, completion: { bluredImage in
		let img = bluredImage
	})
}
